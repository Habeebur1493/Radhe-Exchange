...
const Sidebar = () => {
  ...
  const menuItems = [
    ...
    },
    {
      key: "soccer",
      label: "Soccer",
      icon: "/src/images/soccer-ball.png",
      items: [
        {
          title: "League 1",
  ...
  return (
    <div className="relative min-h-screen sm:min-h-[100vh]">
      {/* Menu Icon */}
  ...
  {menuItems.map(({ key, label, icon, items }) => ( // Extract `icon`
    <li key={key} className="cursor-pointer text-[12px] lg:text-[14px]">
      ...
      >
        <div className="flex items-center">
          {icon && <img src={icon} alt={label} className="w-4 h-4 mr-2" />} {/* Add image safely */}
          {label}
        </div>
