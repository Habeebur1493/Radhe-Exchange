...
const Dashboard = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-100 overflow-x-hidden">
      {/* Navbar */}
      <nav className="bg-[--primary-background] text-white flex justify-between items-center h-14 px-4 shadow-md">
        ...
        {/* Navbar Links */}
        <ul className="hidden md:flex mr-34">
          {["Dashboard", "Clients", "Sport Analysis", "Casino Analysis"].map((item) => (
            <li key={item} className="relative">
              <a
                href="#"
                className="h-14 flex items-center px-2 hover:bg-[#18b0c7] transition-all duration-200 ease-in-out"
              >
                {item}
              </a>
            </li>
          ...
          {["Settlement", "Reports", "Cutting", "Control"].map((item) => (
            <li key={item} className="relative">
              <button className="h-14 flex items-center px-2 hover:bg-[#18b0c7] transition-all duration-200 ease-in-out">
                {item}
                <ChevronDown size={14} className="ml-1" />
              </button>

              {/* Dropdown */}
              <div className="absolute left-0 mt-1 w-40 bg-[#18b0c7] text-gray-800 shadow-md rounded-md hidden group-hover:block transition-all duration-200 ease-in-out">
        ...
        <ul className="hidden md:flex text-sm font-medium">
          <li className="relative">
            ...
            {isDropdownOpen && (
              <div className="absolute left-0 mt-1 w-40 bg-white text-gray-800 shadow-md rounded-md">
        ...
        <div className="bg-white shadow-lg w-60 h-full absolute top-[56px] left-0 p-2">
          <ul className="space-y-2 text-gray-800">
            {["Dashboard", "Clients", "Sport Analysis", "Casino Analysis", "Settlement", "Reports", "Cutting", "Control"].map((item) => (
              <li key={item}>
                <a href="#" className="block hover:bg-gray-200 p-2 rounded transition-all duration-200 ease-in-out">
                  {item}
                </a>
              </li>
            ))}
