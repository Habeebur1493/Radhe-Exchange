...
const Dashboard = () => {
  ...
  return (
    <div className="dashboard min-h-screen bg-gray-100 overflow-x-hidden">
          ...
          <ul className="hidden md:flex space-x-3">
            ...
            {["Settlement"].map((item) => (
              <li key={item} className="relative group">
                ...
                <div className="absolute left-0 mt-0 w-40 bg-[#16a2ba] text-white opacity-1  group-hover:opacity-100 group-hover:block transition-all duration-200 ease-in-out">
                  <a
                    href="#"
      ...
      {/* Header */}
      <header className="bg-[#007182] p-4 shadow-md absolute">
      {/* Dropdown Toggle */}
      <div 
        className="cursor-pointer flex items-center justify-center" 
        onClick={() => setIsOpen(!isOpen)}
      >
        <i className={`ri-arrow-down-s-line bg-white font-bold rounded-full p-1 transition-transform ${isOpen ? "rotate-180" : ""}`}></i>
        
      </div>

      {/* Dropdown Content */}
      {isOpen && (
        <div className="bg-[#007182] text-white transition-all duration-300">
          <div className="grid grid-cols-5 gap-4 mr-4 text-gray-800">
            {[
