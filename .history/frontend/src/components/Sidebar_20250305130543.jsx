...
const Sidebar = () => {
  ...
  const navItems = [
    ...
    },
    {
      key: "Settlement",
      label: "Settlement",
      items: ["User", "Master"], // Updated to a direct array of titles
    },
  ];
  ...
  return (
    <div className="relative">
      ...
      {/* Sidebar */}
      <div
        className={`sidebar fixed top-0 left-0 h-full w-52 mt-14 bg-[--tertiary-background] text-white transition-transform duration-300 ${
        ...
        <ul className="p-2">
          {navItems.map(({ key, label, items }, index) => (
            <li key={key} className="cursor-pointer text-[12px] lg:text-[14px]">
              ...
              >
                {label}{" "}
                {index >= 4 && ( // Show the arrow icon only for items beyond index 3
                  <i
              ...
              <ul
                className={`overflow-hidden  text-black transition-all duration-300 ${
                  dropdowns[key]
