...
const Login = () => {
  ...
  const validateCaptcha = () => {
    if (parseInt(captchaInput) === captcha.result) {
      ...
      setMessage("Captcha verified! You can proceed.");
    } else {
  ...
  const handleLogin = async (e) => {
    ...
    // Check if CAPTCHA is verified before logging in
    if (!isCaptchaValid) {
      setMessage("Please solve the captcha first.");
      return;
    }
    ...
    <div className="main flex items-center justify-center min-h-screen bg-[--primary-background]">
      <div className="bg-black p-6 w-75 h-100 border-[5px] border-[#3d3636]">
        ...
        <form onSubmit={handleLogin} className="space-y-4 text-[14px]">
          ...
          {/* CAPTCHA */}
          <div className="flex justify-center items-center space-x-3 bg-[#d4d4d4] rounded-lg p-2 mb-2">
            ...
            <input
              ...
              disabled={isCaptchaValid} // Disable input after verification
            />
          ...
          {!isCaptchaValid && (
            <button
              ...
              className="w-full btn bg-blue-500 text-white py-2 rounded-lg font-bold mb-2"
            >
              Verify Captcha
            </button>
          )}
          ...
          <button
            ...
            disabled={!isCaptchaValid} // Disable login until CAPTCHA is correct
          >
            LOGIN
          </button>
        </form>
